...
class TPX3BasicConfig:
  ...
  def do_config(self):
    ...
    for phase_num in clk_phase_nums:
      ...
      for clk_period in clk_periods:
        ...
        self.do_read_power("Clk: %d"%(clk_period) + " ToA counter OFF", "\t\t")
        genConfig_register=TPX3_ACQMODE_TOA_TOT | TPX3_FASTLO_ENA | TPX3_GRAYCOUNT_ENA 
        self.tpx.setGenConfig( genConfig_register)
        self.do_read_power("Clk: %d"%(clk_period) + " ToA counter ON","\t\t")
    
    clk_period = test.clk_period
    phase_div, phase = self.get_phase_div(clk_period)
    self.set_clk_period_and_phase(clk_period, phase_div, TPX3_PHASESHIFT_NR_16)
  ...
  def configure_all_pixels(self):
    ...
    a_tot=numpy.fromfile(fbase2+"/a_tot_bin.dat", dtype=dt).reshape((256,256))
    b_tot=numpy.fromfile(fbase2+"/b_tot_bin.dat", dtype=dt).reshape((256,256))
    c_tot=numpy.fromfile(fbase2+"/c_tot_bin.dat", dtype=dt).reshape((256,256))
    t_tot=numpy.fromfile(fbase2+"/t_tot_bin.dat", dtype=dt).reshape((256,256))
    c_toa=numpy.fromfile(fbase2+"/c_toa_bin.dat", dtype=dt).reshape((256,256))
    t_toa=numpy.fromfile(fbase2+"/t_toa_bin.dat", dtype=dt).reshape((256,256))
    d_toa=numpy.fromfile(fbase2+"/d_toa_bin.dat", dtype=dt).reshape((256,256))
    ...
    for i in range(100):
        thr_dac=self.tpx.trackThreshold(target_thr_v=target_thr_v,thr_dac=thr_dac)
...
class test26_vdd_meas(tpx3_test):
  """Test for measuring digital power consumption. """
    ...
    for i in range(nsamples):
      ...
      while not self.temp_within_limits(temperature): 
        ...
        if self.temp_within_limits(temperature):
          ...
          self._msg(ind + "\tVDD Voltage: %8.5f  Current: %8.5f"%(vol,cur))
  ...
  def _execute(self, **keywords):
    try:
      ...
      tpx_config = TPX3BasicConfig(self.tpx, self, False)  # Create a class for configuring
...
class TPX3DataAcq:
  ...
  def do_daq(self, mode = TPX3_ACQMODE_TOA_TOT, max_time = 20.0):
    ...
    self.test.logging.info("Starting Timepix3 data_driven DAQ, mode: %s"%(self.mode_str)) 
    ...
    while not self.finish:
      ...
      if time_elapsed > max_time:
        ...
        self.count_rate_and_record_data(event_counter, time_elapsed, csv_record)
    self.records["DAQ_" + self.mode_str] = csv_record
  
  def do_seq_daq(self, mode = TPX3_ACQMODE_EVT_ITOT, ro_period = 1.0, max_time = 20.00):
    ...
    self.test.logging.info("Starting Timepix3 seq. DAQ, mode: %s"%(self.mode_str)) 
  ...
  def sample_temperature(self, time_temp):
    ...
    if time_temp > 1.0:
      ...
      print "\n"
      self.test.sample_cur_and_temp("DAQ mode %s"%(self.mode_str), "", 1)
      self.nsample += 1
  ...
  def count_rate_and_record_data(self, event_counter, time_elapsed, csv_record):
    if time_elapsed>0 : hitRate=event_counter/time_elapsed
    else: hitRate=0
