                                         ...
                                         bool           is_counterh )//compress)
{
    ...
    for( i=0; i<nbytes/sizeof(u64); ++i, ++pixelpkt )
    {
        ...
        {
            int end = std::min(pix_per_word, (MPX_PIXEL_COLUMNS-index));
            for( j=0; j < end; ++j, ++index )
            {
                //! You need not check index against the MPIX.COLS
                //! if you run out of the row, by definition it's an EOR (or EOF)
                //! except when you loose a UDP packet and have too many MID
                //! packets because you lost a EOR/SOR combo
                //!
                //!
                //! Should be correct index < MPX_PIXEL_COLUMNS
                //!
                //! The end variable is used to calculate the correct number of
                //! pixels in this packet. Should be pix_per_word normally
        ...
        case PIXEL_DATA_EOF:
            //! See the VHDL for more details on what this is.
            flags = int( ((pixelword & ROW_COUNT_MASK) >> ROW_COUNT_SHIFT) );

        case PIXEL_DATA_EOR:
            rownr = int( ((pixelword & ROW_COUNT_MASK) >> ROW_COUNT_SHIFT));

            //! Pretty sure this is fixed in the firmware now.
            /*if( _applyLut ) {
            ...
            //! The idea is that you can't run out of the pixels in the word
            //! before getting to the end of the column
            //!
            //! Some bits are already used by the flags/row count
            //!
            //! 16 nibbles - 1 - 2 - 4 = 9 nibbles, i.e. 36 bits

            if ( MPX_PIXEL_COLUMNS - index <= 36.0/pix_per_word) {
                // Unpack the pixel packet
                ...
                qDebug() << "FramebuilderThreadC::mpx3RawToPixel - data + garbage?";
            }
