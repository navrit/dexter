...
QCstmStepperMotor::QCstmStepperMotor(QWidget *parent)
    : QWidget(parent), ui(new Ui::QCstmStepperMotor) {
  ui->setupUi(this);
  ...
  QStandardItemModel *tvmodel =
      new QStandardItemModel(3, 2, this); // 3 Rows and 2 Columns
...
void QCstmStepperMotor::SetMpx3GUI(Mpx3GUI *p) {
  ...
  _stepper = nullptr;
  connect(ui->stepperUseCalibCheckBox, SIGNAL(clicked(bool)), config,
          SLOT(setStepperConfigUseCalib(bool)));
  connect(config, SIGNAL(UseCalibChanged(bool)), ui->stepperUseCalibCheckBox,
          SLOT(setChecked(bool)));

  connect(ui->accelerationSpinBox, SIGNAL(valueChanged(double)), config,
          SLOT(setStepperConfigAcceleration(double)));
  connect(config, SIGNAL(AccelerationChanged(double)), ui->accelerationSpinBox,
          SLOT(setValue(double)));

  connect(ui->speedSpinBox, SIGNAL(valueChanged(double)), config,
          SLOT(setStepperConfigSpeed(double)));
  connect(config, SIGNAL(SpeedChanged(double)), ui->speedSpinBox,
          SLOT(setValue(double)));

  // When the slider released, talk to the hardware
  QObject::connect(ui->motorDial, SIGNAL(sliderReleased()), this,
                   SLOT(motorDialReleased()));
  QObject::connect(ui->motorDial, SIGNAL(sliderMoved(int)), this,
                   SLOT(motorDialMoved(int)));

  connect(this, SIGNAL(sig_motorsConnected()), _mpx3gui->GetUI()->ctTab,
...
void QCstmStepperMotor::on_stepperMotorCheckBox_toggled(bool checked) {
  ...
  disconnect(ui->stepperCalibrationTableView->model(),
             SIGNAL(itemChanged(QStandardItem *)), _mpx3gui->getConfig(),
             SLOT(setStepperConfigCalib(QStandardItem *)));
  disconnect(ui->accelerationSpinBox, SIGNAL(valueChanged(double)), this,
             SLOT(setAcceleration(double)));
  disconnect(ui->speedSpinBox, SIGNAL(valueChanged(double)), this,
  ...
  if (checked) {
    ...
    ui->speedSpinBox->setValue( 1000.0 ); // Could be .vel
    ...
    ui->accelerationSpinBox->setValue( 20000.0 ); // Could be .acc
    ...
    if (configAcc >= parsMap[motorid].minAcc &&
        configAcc <= parsMap[motorid].maxAcc) {
      ui->accelerationSpinBox->setValue(configAcc);
      setAcceleration(configAcc);
    }
    double configSpeed = _mpx3gui->getConfig()->getStepperSpeed();
    if (configSpeed >= parsMap[motorid].minVel &&
        configSpeed <= parsMap[motorid].maxVel) {
      ui->speedSpinBox->setValue(configSpeed);
      setSpeed(configSpeed);
    }

    QStandardItemModel *model =
        (QStandardItemModel *)ui->stepperCalibrationTableView->model();
  ...
  connect(ui->stepperCalibrationTableView->model(),
          SIGNAL(itemChanged(QStandardItem *)), _mpx3gui->getConfig(),
          SLOT(setStepperConfigCalib(QStandardItem *)));
  connect(ui->accelerationSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setAcceleration(double)));
  connect(ui->speedSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setSpeed(double)));
  connect(ui->currentISpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(setCurrentILimit(double)));
...
void QCstmStepperMotor::on_stepperUseCalibCheckBox_toggled(bool checked) {
  ...
  if (checked) {
    ...
    // If calibration OK
    if (_stepper->SetStepAngleCalibration(int(ui->motorIdSpinBox->value()),
                                          vals)) {
      cout << "[STEP] Calibrated with " << vals.size() << " points." << endl;
    } else {
      // User message
      string messg = "Insufficient points to calibrate or incorrect format.";
      QMessageBox::warning(this, tr("Can't calibrate stepper"),
                           tr(messg.c_str()));
      // force uncheck
...
void QCstmStepperMotor::on_motorGoToTargetButton_clicked() {
  ...
  if (!_stepperThread) {
    ...
    connect(_stepperThread, SIGNAL(finished()), this,
            SLOT(stepperGotoTargetFinished()));
    connect(_stepperThread, SIGNAL(finished()), _mpx3gui->getCT(),
...
void QCstmStepperMotor::stepperGotoTargetFinished() {
  ...
  if (m_stepperTestCurrentStep != 0 &&
    ...
    qDebug() << m_stepperTestCurrentStep << " : "
             << m_stepperTestSequence.size();
    ...
    // Look if we are working calibrated or uncalibrated
    if (ui->stepperUseCalibCheckBox->isChecked()) {
      targetPos = _stepper->AngleToStep(motorId, targetPos);
    }
    _stepper->setTargetPos(motorId, targetPos);
...
void ConfigStepperThread::run() {
  ...
  while (!reachedTarget) {
    ...
    if (curr_pos ==
        _stepperController->getMotorController()->getTargetPos(motorId))
      reachedTarget = true;
    ...
    if (_ui->stepperUseCalibCheckBox->isChecked()) {
      posDisplay = _stepperController->getMotorController()->StepToAngle(
          motorId, double(curr_pos));
      // Update display
  ...
  if (qFuzzyCompare(posDisplay, _ui->targetPosSpinBox->value())) {
    ...
    _ui->motorCurrentPoslcdNumber->setToolTip(
        "Requested target couldn't be reached exactly.");
    qDebug()
        << "[INFO] Requested target could not be reached exactly, target = "
        << _ui->targetPosSpinBox->value() << " | actual = " << posDisplay;
  }
}
