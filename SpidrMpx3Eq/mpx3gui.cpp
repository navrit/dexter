...
QCstmDacs *Mpx3GUI::getDACs()
{
    return _ui->DACsWidget;
}
QCstmConfigMonitoring *Mpx3GUI::getConfigMonitoring()
{
    return _ui->CnMWidget;
}
EnergyConfiguration *Mpx3GUI::getEnergyConfiguration()
{
    return _ui->EnergyConfigurationTab;
}
hdmiConfig *Mpx3GUI::getHdmiConfig()
{
    return _ui->hdmiConfigTab;
}
QCstmCT *Mpx3GUI::getCT()
{
    return _ui->ctTab;
}
QCstmStepperMotor *Mpx3GUI::getStepperMotor()
{
    return _ui->stepperMotorTab;
}
thresholdScan *Mpx3GUI::getTHScan()
{
    return _ui->THScan;
}
        ...
        if (check_folder.exists() && check_folder.isReadable()) {
            ...
            if (autoLoad == "") {
                ...
                reply = QMessageBox::question(this,
                                              "Question",
                                              "Always auto-load the last configuration?",
                ...
                if (autoLoad == "true" || autoLoad == "yes" || autoLoad == "1") {
                    ...
                    emit sig_statusBarAppend(QString("Autoloading last configuration file from "
                    ...
                    emit sig_statusBarClean();
                    emit sig_statusBarAppend(QString("Not autoloading last configuration file from "
                                                     + lastConfigurationPath),
                                             "black");
                }
            }
...
bool Mpx3GUI::establish_connection() {
    ...
    workingSet = new Dataset(chipSize,
                             chipSize,
                             config->getNActiveDevices(),
...
void Mpx3GUI::_initialiseGUITabs()
{
    ...
    if (_gradients.empty()) {
        QMessageBox::critical(this,
                              ...
                              QString(
                                  "Couldn't load the following configuration file: %1 . The "
...
void Mpx3GUI::_connectKeyboardShortcuts()
{
    ...
    _shortcutsSwitchPages.push_back(
        new QShortcut(QKeySequence(tr("Ctrl+2", "Switch to configuration and monitoring")), this));
    _shortcutsSwitchPages.push_back(
        new QShortcut(QKeySequence(tr("Ctrl+3", "Switch to DAC control")), this));
    _shortcutsSwitchPages.push_back(
        new QShortcut(QKeySequence(tr("Ctrl+4", "Switch to Equalization")), this));
    _shortcutsSwitchPages.push_back(
        new QShortcut(QKeySequence(tr("Ctrl+5", "Switch to Threshold Scan")), this));
    _shortcutsSwitchPages.push_back(
        new QShortcut(QKeySequence(tr("Ctrl+6", "Switch to Energy configuration")), this));
    _shortcutsSwitchPages.push_back(
        new QShortcut(QKeySequence(tr("Ctrl+7", "Switch to HDMI Config")), this));
    _shortcutsSwitchPages.push_back(
        new QShortcut(QKeySequence(tr("Ctrl+D, Ctrl+Alt+6", "Switch to Scans")), this));

    _shortcutsSwitchPages.push_back(new QShortcut(QKeySequence(tr("Ctrl+8", "Switch to CT")), this));
    _shortcutsSwitchPages.push_back(
        new QShortcut(QKeySequence(tr("Ctrl+9", "Switch to Stepper Motor Control")), this));
...
void Mpx3GUI::_setupSignalsAndSlots()
{
    ...
    connect(_ui->actionLoad_equalisation_from_folder,
            SIGNAL(triggered()),
            this,
    ...
    connect(_ui->actionSave_Equalization,
            SIGNAL(triggered()),
            _ui->equalizationWidget,
            SLOT(SaveEqualization()));
    ...
    connect(_ui->equalizationWidget,
            &QCstmEqualization::sig_statusBarAppend,
            this,
            &Mpx3GUI::statusBarAppend);
    connect(_ui->equalizationWidget,
            &QCstmEqualization::sig_statusBarClean,
            this,
            &Mpx3GUI::statusBarClean);
    connect(_ui->stepperMotorTab,
            &QCstmStepperMotor::sig_statusBarAppend,
            this,
            &Mpx3GUI::statusBarAppend);

    connect(getConfig(),
            SIGNAL(colourModeChanged(bool)),
            getTHScan(),
    ...
    connect(getConfig(),
            SIGNAL(colourModeChanged(bool)),
            getEnergyConfiguration(),
            SLOT(slot_colourModeChanged(bool)));
    connect(getConfig(),
            SIGNAL(readBothCountersChanged(bool)),
            getDACs(),
    ...
    for (int i = 0; i < _shortcutsSwitchPages.size(); i++) {
        connect(_shortcutsSwitchPages[i],
                &QShortcut::activated,
                this,
...
void Mpx3GUI::_initialiseServers()
{
    ...
    if (!tcpServer->listen(QHostAddress::Any, tcpCommandPort)) {
        ...
        qDebug() << "[ERROR]\tCheck if there is another process already bound to port:"
                 << tcpCommandPort;
        return;
    } else {
    ...
    connect(tcpServer,
            SIGNAL(dataReceived(QString)),
            commandHandlerWrapper,
            SLOT(on_dataReceived(QString)));
    connect(commandHandlerWrapper,
            SIGNAL(responseIsReady(QString)),
            tcpServer,
    ...
    connect(commandHandlerWrapper,
            SIGNAL(imageIsReady(QByteArray, Canvas)),
            dataServer,
            SLOT(on_imageIsReady(QByteArray, Canvas)));
}
...
void Mpx3GUI::save_data(bool requestPath, int frameId, QString selectedFileType)
{
    ...
    if (!requestPath) {
        ...
        filename = QFileDialog::getSaveFileName(this,
                                                tr("Save Data"),
                                                ".",
                                                BIN_FILES ";;" TIFF_FILES ";;" RAW_TIFF_FILES
                                                          ";;" BOTH_TIFF_FILES
                                                          ";;" SPATIAL_TIFF_FILES ";;" ASCII_FILES,
                                                &selectedFilter);
        ...
        if (selectedFilter == BIN_FILES && !filename.toLower().toUtf8().contains(".bin")) {
            filename.append(".bin");
                   ...
                   && !filename.toLower().toUtf8().contains("_spatialCorrected.tiff")) {
            ...
            filename.append("_raw.tiff");
                   ...
                   && !filename.toLower().toUtf8().contains(".tiff")) {
        ...
        const QString path = getVisualization()
        ...
        filename = QString(QDir::cleanPath(path + QDir::separator()
                                           + QString::number(QDateTime::currentMSecsSinceEpoch())));

        //! if saving all frames, append the frame ID too - more than 1 frame may be
        //! saved within 1 ms
        if (getVisualization()->isSaveAllFramesChecked()) { // TODO optimise this, no need to call
                                                            // this every loop
    ...
    if (selectedFilter == BIN_FILES) {
    ...
    } else if (selectedFilter == SPATIAL_TIFF_FILES || selectedFilter == RAW_TIFF_FILES
               || selectedFilter == RAW_TIFF16_FILES || selectedFilter == PGM16_FILES
               || selectedFilter == TIFF_FILES || selectedFilter == BOTH_TIFF_FILES) {
        ...
        foreach (int key, dataset->getThresholds()) {
            ...
            if (selectedFilter == SPATIAL_TIFF_FILES) {
                ...
                tmpFilename = tmpFilename.replace("_spatialCorrected.tiff",
                                                  QString("-th" + QString::number(key)
                                                          + "_spatialCorrected.tiff"));

            } else if (selectedFilter == RAW_TIFF_FILES) {
                frame = dataset->getFullImageAsArrayWithLayout(key, 4);
                tmpFilename = tmpFilename.replace("_raw.tiff",
                                                  QString("-th" + QString::number(key)
                                                          + "_raw.tiff"));
                ...
                frame = dataset->getFullImageAsArrayWithLayout(key, 2);
                tmpFilename = tmpFilename.replace("_raw.pgm",
                                                  QString("-th" + QString::number(key)
                                                          + "_raw.pgm"));
                QtConcurrent::run(dataControllerThread,
                                  &DataControllerThread::savePGMParallel,
                                  tmpFilename,
                                  frame);
                continue;
                ...
                tmpFilename = tmpFilename.replace(".tiff",
                                                  QString("-th" + QString::number(key) + ".tiff"));

            } else if (selectedFilter == BOTH_TIFF_FILES) {
                frame = dataset->getFullImageAsArrayWithLayout(key, 4);
                tmpFilename = tmpFilename.replace("_both.tiff",
                                                  QString("-th" + QString::number(key)
                                                          + "_raw.tiff"));
                QtConcurrent::run(dataControllerThread,
                                  &DataControllerThread::saveTIFFParallel,
                                  tmpFilename,
                                  frame);
...
bool Mpx3GUI::load_config() {
    ...
    if (!_loadConfigRemotely) {
        _configPath = QFileDialog::getOpenFileName(this,
                                                   tr("Load config (DACs)"),
                                                   tr("."),
                                                   tr("json files (*.json)"));
    }
...
void Mpx3GUI::open_data() {
    ...
    if (selectedFilter.contains("Native binary")) {
        ...
        getDataset()->fromASCIIMatrixGetSizeAndLayers(toOpenFile, &x, &y, &framesPerLayer);
    ...
    // required signals
    QRectF bbox = getDataset()->getBoundingBox();
    emit sizeChanged(int(bbox.width() * getDataset()->x()),
...
void Mpx3GUI::on_actionAbout_triggered(bool)
{
    ...
    QString frameworks = QString("This program uses the following frameworks and libraries with "
    ...
    QString msg = QString("Version: ") + _softwareVersion + newLine + newLine
                  ...
                  + m_SPIDRSoftwareVersion + newLine + m_numberOfChipsFound + newLine + newLine
                  + QString("Authors: ")
