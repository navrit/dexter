...
QCstmEqualization *Mpx3GUI::getEqualization() {
  return _ui->equalizationWidget;
}
QCstmGLVisualization *Mpx3GUI::getVisualization() {
  return _ui->visualizationGL;
}
QCstmDacs *Mpx3GUI::getDACs() { return _ui->DACsWidget; }
QCstmConfigMonitoring *Mpx3GUI::getConfigMonitoring() { return _ui->CnMWidget; }
EnergyConfiguration *Mpx3GUI::getEnergyConfiguration() {
  return _ui->EnergyConfigurationTab;
}
...
Mpx3GUI::Mpx3GUI(QWidget *parent) : QMainWindow(parent), _ui(new Ui::Mpx3GUI) {
...
void Mpx3GUI::resize(int x, int y) {
  ...
  emit sizeChanged(int(bbox.width() * x),
                   int(bbox.height() * y)); // goes to qcstmglplot
}

/**
 * @brief For FPM, assumes that the int* data in in the circular buffer, so you
 * can take and keep it briefly
 * @param data
 * @param layer
 */
...
Gradient *Mpx3GUI::getGradient(int index) { return _gradients.at(index); }
...
void Mpx3GUI::sendingShutter() {
  ...
  connect(_shutterOpenTimer, SIGNAL(timeout()), this,
          SLOT(shutterOpenTimer_timeout()));
  connect(_shutterCloseTimer, SIGNAL(timeout()), this,
          SLOT(shutterCloseTimer_timeout()));
  _timerStop = false;
...
int Mpx3GUI::getStepperMotorPageID() { return __stepperMotor_page_Id; }
...
void Mpx3GUI::loadLastConfiguration() {
  ...
  if (check_file.exists() && check_file.isFile() && check_file.isReadable()) {
    ...
    QString lastConfigurationPath =
        settings.value("last_configuration_path", "").toString();
    QString autoLoad =
        settings.value("auto_load_last_configuration", "").toString();
    ...
    if (check_folder.exists() && check_folder.isReadable()) {
      ...
      if (autoLoad == "") {
        //! Make dialog asking if you want to always auto-load the last
        //! configuration
        QMessageBox::StandardButton reply;
        reply = QMessageBox::question(
            this, "Question", "Always auto-load the last configuration?",
        ...
        if (autoLoad == "true" || autoLoad == "yes" || autoLoad == "1") {
          ...
          config->fromJsonFile(lastConfigurationPath + QDir::separator() +
                               "config.json");
          _ui->DACsWidget->PopulateDACValues();

          emit sig_statusBarClean();
          emit sig_statusBarAppend(
              QString("Autoloading last configuration file from " +
                      lastConfigurationPath),
              "green");
          ...
          emit sig_statusBarClean();
          emit sig_statusBarAppend(
              QString("Not autoloading last configuration file from " +
                      lastConfigurationPath),
              "black");
        }
      }
      ...
      if (lastConfigurationPath == "") {
        ...
        msg = QString("Auto-load configuration folder does not exist : " +
                      lastConfigurationPath);
      }
      qDebug().noquote() << msg;
      emit sig_statusBarClean();
...
void Mpx3GUI::stopTriggerTimers() {
  ...
  disconnect(_shutterOpenTimer, SIGNAL(timeout()), this,
             SLOT(shutterOpenTimer_timeout()));
  disconnect(_shutterCloseTimer, SIGNAL(timeout()), this,
             SLOT(shutterCloseTimer_timeout()));
  _timerStop = true;
...
Mpx3GUI *Mpx3GUI::getInstance() { return mpx3GuiInstance; }
...
bool Mpx3GUI::establish_connection() {
  ...
  if (spidrcontrol->isConnected()) {
    ...
    int ipaddr;
    // This call takes device number 0 because it is not really addressed to a
    // chip in particular
    if (spidrcontrol->getIpAddrDest(0, &ipaddr)) {
      msg += " - Computer IP: " + QString::number((ipaddr >> 24) & 0xFF) + "." +
    ...
    QMessageBox::critical(
        ...
        tr("Could not establish a connection to the SPIDR "
           "controller.\n\nStatus: %1"
           "\nError message: %2"
           "\n\n"
           "If unsure about the network setup you can listen to the SPIDR "
           "during "
  ...
  if (spidrcontrol->getSoftwVersion(&version)) {
    m_SPIDRSoftwareVersion =
        ...
        QString(spidrcontrol->versionToString(version).c_str()) + QString("\n");
  }
  ...
  for (int i = 0; i < 4; ++i) {
    msg +=
        " " + QString::fromStdString(_spidrdaq->ipAddressString(/*i*/)) + " ";
  }
  ...
  if (_spidrdaq->hasError()) {
    QMessageBox::critical(
        ...
        tr("Couldn't open an UDP connection. Error message:\n\n%1"
           "\n\nThis could be due to:\n- A program using the required UDP "
           "ports."
  ...
  getConfigMonitoring()->OperationModeSwitched(
  ...
  int chipSize =
      config->getColourMode() ? __matrix_size_x / 2 : __matrix_size_x;
  workingSet = new Dataset(
      chipSize, chipSize, config->getNActiveDevices(),
      config->getPixelDepth()); // TODO: get framesize from config, load offsets
  ...
  for (int i = 0; i < activeDevices.size(); i++) {
    ...
    workingSet->setOrientation(
        i, _MPX3RX_ORIENTATION[std::size_t(activeDevices[i])]);
  }
  workingSet->computeBoundingBox();
...
void Mpx3GUI::statusBarAppend(QString mess, QString colorString) {
  ...
  // qDebug() << "mess:" << messRect.width() << "stat:" << statusRect.width() <<
  // " | " << m_statusBarMessageString << "\n";
...
void Mpx3GUI::_initialiseGUITabs() {
  ...
  if (_gradients.empty()) {
    QMessageBox::critical(
        this, "Loading configuration error",
        QString("Couldn't load the following configuration file: %1 . The "
                "program won't start. Place the file in the suggested relative "
                "path and run again. You are running the program from \"%2\"")
            .arg(heatmapsFile)
...
void Mpx3GUI::_handleConfigLoading() {
  ...
  if (!configFileLoaded) {
    qDebug() << "[" << _generalSettings->getConfigPath()
             << "] does not exist. Config file loaded from defualt path.";
  ...
  if (!configFileLoaded) {
    QMessageBox::critical(
        ...
        QString("Couldn't load the following configuration file: %1. The "
                "program won't start. Place the file in the suggested relative "
                "path and run again. You are running the program from \"%2\"")
            .arg(_generalSettings->getConfigPath())
            .arg(QDir::currentPath()));
    _armedOk = false; // it won't let the application go into the event loop
    return;
  }
...
void Mpx3GUI::_connectKeyboardShortcuts() {
  ...
  _shortcutsSwitchPages.push_back(
      new QShortcut(QKeySequence(tr("Ctrl+1", "Switch to viewer")), this));
  _shortcutsSwitchPages.push_back(new QShortcut(
      QKeySequence(tr("Ctrl+2", "Switch to configuration and monitoring")),
      this));
  _shortcutsSwitchPages.push_back(
      new QShortcut(QKeySequence(tr("Ctrl+3", "Switch to DAC control")), this));
  _shortcutsSwitchPages.push_back(new QShortcut(
      QKeySequence(tr("Ctrl+4", "Switch to Equalization")), this));
  _shortcutsSwitchPages.push_back(new QShortcut(
      QKeySequence(tr("Ctrl+5", "Switch to Threshold Scan")), this));
  _shortcutsSwitchPages.push_back(new QShortcut(
      QKeySequence(tr("Ctrl+6", "Switch to Energy configuration")), this));
  _shortcutsSwitchPages.push_back(
      new QShortcut(QKeySequence(tr("Ctrl+7", "Switch to HDMI Config")), this));
  _shortcutsSwitchPages.push_back(new QShortcut(
      QKeySequence(tr("Ctrl+D, Ctrl+Alt+6", "Switch to Scans")), this));

  _shortcutsSwitchPages.push_back(
      new QShortcut(QKeySequence(tr("Ctrl+8", "Switch to CT")), this));
  _shortcutsSwitchPages.push_back(new QShortcut(
      QKeySequence(tr("Ctrl+9", "Switch to Stepper Motor Control")), this));
  ...
  connect(shortcutStart, SIGNAL(activated()), _ui->visualizationGL,
          SLOT(shortcutStart()));
  QShortcut *shortcutIntegrate = new QShortcut(QKeySequence("i"), this);
  connect(shortcutIntegrate, SIGNAL(activated()), _ui->visualizationGL,
          SLOT(shortcutIntegrate()));
  QShortcut *shortcutIntegrateToggle =
      new QShortcut(QKeySequence("Alt+i"), this);
  connect(shortcutIntegrateToggle, SIGNAL(activated()), _ui->visualizationGL,
          SLOT(shortcutIntegrateToggle()));
  QShortcut *shortcutFrameLength = new QShortcut(QKeySequence("l"), this);
  connect(shortcutFrameLength, SIGNAL(activated()), _ui->visualizationGL,
          SLOT(shortcutFrameLength()));
  QShortcut *shortcutFrameNumber = new QShortcut(QKeySequence("f"), this);
  connect(shortcutFrameNumber, SIGNAL(activated()), _ui->visualizationGL,
          SLOT(shortcutFrameNumber()));

  // Connections to Configuration and settings
  QShortcut *shortcutGainModeSLGM = new QShortcut(QKeySequence("g, 4"), this);
  connect(shortcutGainModeSLGM, SIGNAL(activated()), _ui->CnMWidget,
          SLOT(shortcutGainModeSLGM()));
  QShortcut *shortcutGainModeLGM = new QShortcut(QKeySequence("g, 3"), this);
  connect(shortcutGainModeLGM, SIGNAL(activated()), _ui->CnMWidget,
          SLOT(shortcutGainModeLGM()));
  QShortcut *shortcutGainModeHGM = new QShortcut(QKeySequence("g, 2"), this);
  connect(shortcutGainModeHGM, SIGNAL(activated()), _ui->CnMWidget,
          SLOT(shortcutGainModeHGM()));
  QShortcut *shortcutGainModeSHGM = new QShortcut(QKeySequence("g, 1"), this);
  connect(shortcutGainModeSHGM, SIGNAL(activated()), _ui->CnMWidget,
          SLOT(shortcutGainModeSHGM()));

  QShortcut *shortcutCSMOff = new QShortcut(QKeySequence("c, 0"), this);
  connect(shortcutCSMOff, SIGNAL(activated()), _ui->CnMWidget,
          SLOT(shortcutCSMOff()));
  QShortcut *shortcutCSMOn = new QShortcut(QKeySequence("c, 1"), this);
  connect(shortcutCSMOn, SIGNAL(activated()), _ui->CnMWidget,
          SLOT(shortcutCSMOn()));
}

void Mpx3GUI::_setupSignalsAndSlots() {
  connect(_ui->actionLoad_Equalization, SIGNAL(triggered()), this,
          SLOT(LoadEqualization()));
  connect(_ui->actionLoad_equalisation_from_folder, SIGNAL(triggered()), this,
          SLOT(loadEqualisationFromPath()));
  ...
  connect(_ui->actionSumming, SIGNAL(triggered()), this,
          SLOT(set_mode_integral()));
  connect(_ui->actionDiscrete, SIGNAL(triggered()), this,
          SLOT(set_mode_normal()));

  connect(_ui->actionSave_data, SIGNAL(triggered(bool)), this,
          SLOT(save_data(bool)));
  connect(_ui->actionSave_Equalization, SIGNAL(triggered()),
          _ui->equalizationWidget, SLOT(SaveEqualization()));
  ...
  connect(_ui->actionClear_configuration, SIGNAL(triggered()), this,
          SLOT(clear_configuration()));

  connect(_ui->actionDeveloper_mode, SIGNAL(triggered()), this,
          SLOT(developerMode()));

  // Change me when adding extra views
  // Inform every module of changes in connection status
  connect(this, SIGNAL(ConnectionStatusChanged(bool)), _ui->DACsWidget,
          SLOT(ConnectionStatusChanged(bool)));
  connect(this, SIGNAL(ConnectionStatusChanged(bool)), _ui->equalizationWidget,
          SLOT(ConnectionStatusChanged(bool)));
  connect(this, SIGNAL(ConnectionStatusChanged(bool)), _ui->visualizationGL,
          SLOT(ConnectionStatusChanged(bool)));
  connect(this, SIGNAL(ConnectionStatusChanged(bool)), _ui->stepperMotorTab,
          SLOT(ConnectionStatusChanged(bool)));
  connect(this, SIGNAL(ConnectionStatusChanged(bool)), _ui->CnMWidget,
          SLOT(ConnectionStatusChanged(bool)));
  connect(this, SIGNAL(ConnectionStatusChanged(bool)),
          _ui->EnergyConfigurationTab, SLOT(ConnectionStatusChanged(bool)));
  connect(this, SIGNAL(ConnectionStatusChanged(bool)), _ui->THScan,
          SLOT(ConnectionStatusChanged(bool)));
  connect(this, SIGNAL(ConnectionStatusChanged(bool)), _ui->hdmiConfigTab,
          SLOT(ConnectionStatusChanged(bool)));
  //    connect( this, SIGNAL( ConnectionStatusChanged(bool) ), _ui->ctTab,
  //    SLOT( ConnectionStatusChanged(bool) ) );
  connect(this, SIGNAL(ConnectionStatusChanged(bool)), _ui->stepperMotorTab,
          SLOT(ConnectionStatusChanged(bool)));
  connect(this, &Mpx3GUI::ConnectionStatusChanged, this,
          &Mpx3GUI::onConnectionStatusChanged);
  ...
  connect(_ui->equalizationWidget, &QCstmEqualization::sig_statusBarAppend,
          this, &Mpx3GUI::statusBarAppend);
  connect(_ui->equalizationWidget, &QCstmEqualization::sig_statusBarClean, this,
          &Mpx3GUI::statusBarClean);
  connect(_ui->stepperMotorTab, &QCstmStepperMotor::sig_statusBarAppend, this,
          &Mpx3GUI::statusBarAppend);

  connect(getConfig(), SIGNAL(colourModeChanged(bool)), getTHScan(),
          SLOT(slot_colourModeChanged(bool)));
  connect(getConfig(), SIGNAL(colourModeChanged(bool)), getDACs(),
          SLOT(slot_colourModeChanged(bool)));
  connect(getConfig(), SIGNAL(colourModeChanged(bool)),
          getEnergyConfiguration(), SLOT(slot_colourModeChanged(bool)));
  connect(getConfig(), SIGNAL(readBothCountersChanged(bool)), getDACs(),
          SLOT(slot_readBothCounters(bool)));
  connect(getConfig(), SIGNAL(readBothCountersChanged(bool)),
          getEnergyConfiguration(), SLOT(slot_readBothCounters(bool)));

  for (int i = 0; i < _shortcutsSwitchPages.size(); i++) {
    connect(_shortcutsSwitchPages[i], &QShortcut::activated, this,
            &Mpx3GUI::on_shortcutsSwitchPages);
  }
}
...
void Mpx3GUI::_initialiseServers() {
  ...
  if (!tcpServer->listen(QHostAddress::Any, tcpCommandPort)) {
    qWarning() << "[ERROR]\tTCP Command server cannot listen on port:"
               << tcpCommandPort;
    qDebug()
        << "[ERROR]\tCheck if there is another process already bound to port:"
        << tcpCommandPort;
    return;
  } else {
    qDebug().nospace() << "[INFO]\tTCP Command server listening on \"tcp://*:"
                       << tcpCommandPort << "\"";
  }
  commandHandlerWrapper = new CommandHandlerWrapper(this);
  connect(tcpServer, SIGNAL(dataReceived(QString)), commandHandlerWrapper,
          SLOT(on_dataReceived(QString)));
  connect(commandHandlerWrapper, SIGNAL(responseIsReady(QString)), tcpServer,
          SLOT(on_responseIsReady(QString)));

  //! Diamond - Merlin interface data socket
  dataServer = new TcpServer;
  if (!dataServer->listen(QHostAddress::Any, tcpDataPort)) {
    qWarning() << "[ERROR]\tTCP Data server cannot listen on port:"
               << tcpDataPort;
    qDebug()
        << "[ERROR]\tCheck if there is another process already bound to port:"
        << tcpDataPort;
    ...
    qDebug().nospace() << "[INFO]\tTCP Data server listening on \"tcp://*:"
                       << tcpDataPort << "\"";
  }
  connect(commandHandlerWrapper, SIGNAL(imageIsReady(QByteArray, Canvas)),
          dataServer, SLOT(on_imageIsReady(QByteArray, Canvas)));
}
...
//! Debugging function to generate test patterns, used to verify files are being
//! saved correctly Note: This is configured for the current orientation scheme
//! for 4 chips.
void Mpx3GUI::generateFrame() {
  ...
  for (int k = 0; k < chips; ++k) {
    //! Only do this for the first threshold (layer) because it will always be
    //! there and cannot mess with the other thresholds if they exist...
    //!
...
QPoint Mpx3GUI::getSize() { return getDataset()->getSize(); }
...
int Mpx3GUI::getX() { return getDataset()->getSize().x(); }
int Mpx3GUI::getY() { return getDataset()->getSize().y(); }

int Mpx3GUI::getFrameCount() { return getDataset()->getFrameCount(); }

QString Mpx3GUI::getLoadButtonFilename() { return _loadButtonFilenamePath; }
...
void Mpx3GUI::saveMetadataToJSON(QString filename) {
  if (filename.toLower().contains(".bin")) {
    ...
    qDebug() << "[ERROR]\tFailed writing the JSON file, input name didn't "
                "include .tiff, .bin or .txt";
    return;
  }
...
*/
  ...
  objParent.insert("Kernel_typeAndVersion",
                   (QSysInfo::kernelType() + QSysInfo::kernelVersion()));
  objParent.insert("Compile_dateTime", compileDateTime);
  objParent.insert("C++_version",
                   QString::fromStdString(to_string(__cplusplus)));
  objParent.insert("Unix_time_s", (QDateTime::currentMSecsSinceEpoch() / 1000));
  objParent.insert("Date_time_local",
                   (QDateTime::currentDateTime().toString(Qt::ISODate)));
  objParent.insert("Date_time_UTC",
  ...
  if (!saveFile.open(QIODevice::WriteOnly)) {
    qDebug() << "[WARN]\tJSON file CANNOT be opened as QIODevice::WriteOnly, "
                "aborting";
...
void Mpx3GUI::_loadEqualizationFromGeneralSettings() {
  ...
  if (eqDir.exists()) {
    getEqualization()->LoadEqualisation(
        false, false, _generalSettings->getEqualisationPath());
    qDebug() << "[INFO]\tEqualization loaded from"
             << _generalSettings->getEqualisationPath();
  }
}
...
bool Mpx3GUI::_loadConfigsFromGeneralSettings() {
  ...
  QStringList pathList =
      _generalSettings->getConfigPath().split(QDir::separator());
...
void Mpx3GUI::updateEnergyCalibratorParameters() {
  const int threshold =
      0; /* TODO implement the threshold properly, this is just a quick fix */
  _generalSettings->readSetting();
  for (int chip = 0; chip < __max_number_of_chips; chip++) {
    _energyCalibrator->setSlope(chip, threshold,
                                _generalSettings->getSlope(chip));
    _energyCalibrator->setOffset(chip, threshold,
                                 _generalSettings->getOffset(chip));
  }
}
...
void Mpx3GUI::save_data(bool requestPath, int frameId,
  ...
  //! Manual or autosave
  if (!requestPath) {
    QString initial_save_path = ".";
    QFileInfo check_file(settingsFile);
    QSettings settings(settingsFile, QSettings::NativeFormat);
    if (check_file.exists() && check_file.isFile() && check_file.isReadable()) {
      QString lastSavePath = settings.value("last_save_path", "").toString();
      //! Check folder exists before continuing
      QFileInfo check_folder(lastSavePath);
      if (check_folder.exists() && check_folder.isReadable()) {
        initial_save_path = lastSavePath;
      }
    }

    //! Native format - User dialog
    filename = QFileDialog::getSaveFileName(
        this, tr("Save Data"), initial_save_path,
        TIFF_FILES ";;" TIFF16_FILES ";;" PGM16_FILES ";;" ASCII_FILES
                   ";;" BIN_FILES,
        &selectedFilter);
    ...
    initial_save_path = QFileInfo(filename).path();
    settings.setValue("last_save_path", initial_save_path);

    if (selectedFilter == BIN_FILES &&
        !filename.toLower().toUtf8().contains(".bin")) {
      filename.append(".bin");
    } else if (selectedFilter == TIFF_FILES &&
               !filename.toLower().toUtf8().contains("_raw.tiff")) {
      filename.append("_raw.tiff");
    } else if (selectedFilter == TIFF16_FILES &&
               !filename.toLower().toUtf8().contains("_raw.tiff")) {
      filename.append("_raw.tiff");
    } else if (selectedFilter == PGM16_FILES &&
               !filename.toLower().toUtf8().contains("_raw.pgm")) {
      filename.append("_raw.pgm");
    } else if (selectedFilter == ASCII_FILES &&
               !filename.toLower().toUtf8().contains(".txt")) {
      filename.append(".txt");
    }
    ...
    const QString path =
        getVisualization()
            ->getsaveLineEdit_Text(); // TODO optimise this, no need to call
                                      // this every loop
    //! Build the filename+path string up by adding  "/", the current UTC date
    //! in ISO format and a file extension
    filename = QString(
        QDir::cleanPath(path + QDir::separator() +
                        QString::number(QDateTime::currentMSecsSinceEpoch())));

    //! if saving all frames, append the frame ID too - more than 1 frame may be
    //! saved within 1 ms
    if (getVisualization()
            ->isSaveAllFramesChecked()) { // TODO optimise this, no need to call
                                          // this every loop
  ...
  //! Send data to be saved by the relevant function with the correct arguments
  //! etc.
  ...
  } else if (selectedFilter == TIFF_FILES || selectedFilter == TIFF16_FILES ||
             selectedFilter == PGM16_FILES) {
    //! This is a shitty way of doing it but whatever the disk is still the
    //! bottleneck... maybe fix it later

    foreach (int key, dataset->getThresholds()) {
      QString tmpFilename = unmodifiedFilename;
      Canvas frame;

      if (selectedFilter == TIFF_FILES) {
        frame = dataset->getFullImageAsArrayWithLayout(key, 4);
        tmpFilename = tmpFilename.replace(
            "_raw.tiff", QString("-th" + QString::number(key) + "_raw.tiff"));
        ...
        frame = dataset->getFullImageAsArrayWithLayout(key, 2);
        tmpFilename = tmpFilename.replace(
            "_raw.pgm", QString("-th" + QString::number(key) + "_raw.pgm"));
        QtConcurrent::run(dataControllerThread,
                          &DataControllerThread::savePGMParallel, tmpFilename,
                          frame);
        continue;
      }

      QtConcurrent::run(dataControllerThread,
                        &DataControllerThread::saveTIFFParallel, tmpFilename,
                        frame);
    }
  } else if (selectedFilter == ASCII_FILES) {
...
void Mpx3GUI::save_config() {
  QString filename = QFileDialog::getSaveFileName(
      this, tr("Save config"), tr("."), tr("json files (*.json)"));
...
bool Mpx3GUI::load_config() {
  ...
  if (!_loadConfigRemotely) {
    _configPath = QFileDialog::getOpenFileName(
        this, tr("Load config (DACs)"), tr("."), tr("json files (*.json)"));
  }
...
void Mpx3GUI::open_data() {
  ...
  QString filename = QFileDialog::getOpenFileName(
      this, tr("Read Data"), tr("."),
      tr("Native binary files (*.bin);;ASCII matrix (*.txt)"), &selectedFilter);
  ...
  if (selectedFilter.contains("Native binary")) {
    ...
    getDataset()->fromASCIIMatrixGetSizeAndLayers(toOpenFile, &x, &y,
                                                  &framesPerLayer);
  ...
  QRectF bbox = getDataset()->getBoundingBox();
  emit sizeChanged(
      int(bbox.width() * getDataset()->x()),
      int(bbox.height() * getDataset()->y())); // goes to qcstmglplot
  emit reload_all_layers();
  ...
  QMessageBox::StandardButton reply = QMessageBox::question(
      this, tr("Specify data"), tr("Is this data corrected?"),
      QMessageBox::Yes | QMessageBox::No);
  if (reply == QMessageBox::Yes)
    getDataset()->setCorrected(true);
  else
    getDataset()->setCorrected(false);

  return;
}

void Mpx3GUI::open_data_with_path(bool saveOriginal, bool requestPath,
                                  QString path) {
  QString filename;
  if (!requestPath) {
    filename = QFileDialog::getOpenFileName(
        this, tr("Read Data"), tr("."),
        tr("Native binary files (*.bin);;ASCII matrix (*.txt)"));
  } else {
    filename = path;
...
void Mpx3GUI::clear_configuration() {
  //! None of the following code should be run unless connected (Connect button
  //! is visible)
  if (_ui->actionConnect->isVisible()) {
    sig_statusBarAppend(tr("Clear equalisation only works when connected"),
                        "black");
    return;
  }
  // Clear adjustment bits
  QMessageBox::StandardButton ans =
      QMessageBox::question(this, tr("Clear configuration"),
                            tr("The adjustment matrix and the pixel mask will "
                               "be cleared.  Continue ?"));
  ...
  if (_ui->equalizationWidget) {
    ...
    for (int i = 0; i < ndev; i++) {
      ...
      if (_ui->equalizationWidget->GetEqualizationResults(i)) {
        qDebug() << "[INFO] clearing adjustment bits and mask for devId : "
                 << i;
...
void Mpx3GUI::on_actionExit_triggered() {
  ...
  if (getVisualization()->DataTakingThreadIsRunning()) { // This means there's a
                                                         // thread ongoing

    if (!getVisualization()
             ->DataTakingThreadIsIdling()) { // actually taking data
      getVisualization()->StopDataTakingThread();
      QMessageBox::warning(this, tr("Exit - pending actions"),
...
void Mpx3GUI::on_actionConnect_triggered() {
  ...
  if (getConfig()->isConnected()) {
    // Reset _lastTriedIPAddress so that this function can auto search IP
    // Addresses without restarting the program
    _lastTriedIPAddress = "";
  }
}
...
void Mpx3GUI::on_actionAbout_triggered(bool) {
  ...
  QString frameworks =
      QString("This program uses the following frameworks and libraries with "
              "their respective licences listed:") +
      newLine + QString("<ul>") +
      QString("<li> Qt - Commercial - <a "
              "href='https://doc.qt.io/qt-5/licensing.html'>doc.qt.io/qt-5/"
              "licensing.html</a> </li>") +
      QString("<li> Boost - Boost - <a "
              "href='http://www.boost.org/LICENSE_1_0.txt'>boost.org/"
              "LICENSE_1_0.txt</a> </li>") +
      QString("<li> LibTiff - BSD like - <a "
              "href='http://simplesystems.org/libtiff/"
              "misc.html'>simplesystems.org/libtiff/misc.html</a> </li>") +
      QString("</ul>") + newLine +
      QString("For the BSD licensed software, see the BSD license : <a "
              "href='https://opensource.org/licenses/"
              "BSD-3-Clause'>opensource.org/licenses/BSD-3-Clause</a>");

  QString msg =
      QString("Version: ") + _softwareVersion + newLine + newLine +
      QString("Compiled: ") + compileDateTime + newLine + QString("C++: ") +
      QString::fromStdString(to_string(__cplusplus)) + newLine +
      m_SPIDRControllerVersion + newLine + m_SPIDRFirmwareVersion + newLine +
      m_SPIDRSoftwareVersion + newLine + m_numberOfChipsFound + newLine +
      newLine + QString("Authors: ") +
      QString("Navrit Bal (2016-), Bram Bouwens (2018-), Kiavash Mortezavi "
              "Matin (2018-), John Idarraga (2014-2017), Amber van Keeken "
              "(2016), Roel Deckers, Cyrano Chatziantoniou") +
      newLine + newLine + QString("Contributors: ") +
...
void Mpx3GUI::shutterOpenTimer_timeout() {
  ...
  if (getConfig()->getTriggerDowntime_ms_64() >= INT_MAX) {
    qDebug() << "[WARNING]\tShutter close timer argument has lost precision "
                "and the maximum value has been limited to"
             << INT_MAX;
  }
}
...
void Mpx3GUI::shutterCloseTimer_timeout() {
  ...
  if (config->getTriggerDowntime_ms_64() >= INT_MAX) {
    qDebug() << "[WARNING]\tShutter open timer argument has lost precision and "
                "the maximum value has been limited to"
             << INT_MAX;
  }
