...
public:
	...
	QRectF m_cutoff;
	int m_requiredWidth = 1, m_nSteps = 25, m_dashLength = 10, m_subDashCount = 0, m_subDashLength = 5, m_margin = 3;
	int m_orientation = orientationLeft;
	void recomputeDisplayRange();
	void paintLeft();
	void paintTop();
public:
    explicit QCstmRuler(QWidget *parent = nullptr);
	...
	signals:
public slots:
void set_cutoff(QRectF newCutoff){
	m_cutoff = newCutoff;
}
void set_cutoff(QPoint newCutoff){
	m_cutoff = QRectF(0,0,newCutoff.x(), newCutoff.y());
}

void set_cutoff(int x, int y){
	set_cutoff(QPoint(x,y));
}
