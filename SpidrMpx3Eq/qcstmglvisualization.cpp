...
QCstmGLVisualization::QCstmGLVisualization(QWidget *parent)
    : QWidget(parent), ui(new Ui::QCstmGLVisualization) {
  ...
  connect(ui->logCheckBox, SIGNAL(clicked(bool)), this,
          SLOT(on_logscale(bool)));
...
QCstmGLVisualization::~QCstmGLVisualization() { delete ui; }

QCstmGLVisualization *QCstmGLVisualization::getInstance() {
  return qCstmGLVisualizationInst;
}

void QCstmGLVisualization::setThresholdsVector(int chipId, int threshold,
                                               int DAC_value) {
  if (chipId >= 0 && chipId < __max_number_of_chips && threshold >= 0 &&
      threshold < __max_number_of_thresholds) {
    _thresholdsVector[chipId][threshold] = DAC_value;
  } else {
    if (threshold >= 0 && threshold < __max_number_of_thresholds) {
      qDebug() << "[ERROR]\tOut of bounds --> chip =" << chipId
               << "| threshold =" << threshold << "| DAC_value =" << DAC_value;
    }
  }
}
...
void QCstmGLVisualization::refreshScoringInfo() {
  ...
  /* Not necessary to do this, nFramesKept is not modified in DataTakingThread
  for colour mode
  ...
  if (lostFrames > 0 && nFramesReceived > 0) {
    frameCounterLabel +=
        QString("<font color=\"red\">(%1)</font>").arg(lostFrames);
  }
  //! Only add this part of the string if a finite number of frames was
  //! requested.
  if (nTriggers > 0) {
...
//! Calling reload_all_layers() directly as a public slot is somehow different
//! to calling it through this public non-slot wrapper
void QCstmGLVisualization::drawFrameImage() { reload_all_layers(); }
...
void QCstmGLVisualization::rewindHistoLimits() {
  ...
  _percentileRange =
      QCPRange(0.025, 0.975); // These instead are reasonable percentile cuts
...
bool QCstmGLVisualization::isBusyDrawing() { return _busyDrawing; }

void QCstmGLVisualization::updateETA() {

  // Recalculate ETA first
  //_estimatedETA = _mpx3gui->getConfig()->getTriggerPeriodMS() *
  //_mpx3gui->getConfig()->getNTriggers(); // ETA in ms _estimatedETA +=
  //_estimatedETA * __networkOverhead; // add ~10% network overhead.  FIXME  to
  //be calculated at startup

  if (_etatimer && !_infDataTaking) {
    // show eta in display
    // h must be in the range 0 to 23, m and s must be in the range 0 to 59, and
    // ms must be in the range 0 to 999.
    QTime n(0, 0, 0); // n == 00:00:00
    QTime t(0, 0, 0);
    int diff = _estimatedETA - _etatimer->elapsed();
    if (diff > 0)
      t = n.addMSecs(_estimatedETA - _etatimer->elapsed());
...
void QCstmGLVisualization::ConfigureGUIForDataTaking() {
  ...
  _mpx3gui->getDataset()->setPixelDepthBits(
      _mpx3gui->getConfig()->getPixelDepth());
  ...
  _mpx3gui->getHdmiConfig()->setEnabled(false);
}
...
void QCstmGLVisualization::ConfigureGUIForIdling() {
  ...
  _mpx3gui->getHdmiConfig()->setEnabled(true);
}
...
void QCstmGLVisualization::CalcETA() {
  ...
  if (_mpx3gui->getConfig()->getOperationMode() ==
    ...
    _estimatedETA = int(_mpx3gui->getConfig()->getTriggerPeriod_ms() *
                        _mpx3gui->getConfig()->getNTriggers()); // ETA in ms.
    if (_mpx3gui->getConfig()->getTriggerPeriod() < LONG_PERIOD_US)
      _estimatedETA += int(_estimatedETA * __overhead); // add 1% overhead
  } else {
    double period_ms =
        (1. / double((_mpx3gui->getConfig()->getContRWFreq()))) * 1000;
    _estimatedETA =
        int(period_ms * _mpx3gui->getConfig()->getNTriggers()); // ETA in ms.
    _estimatedETA += int(_estimatedETA * __overhead); // add 1% overhead
  }
}

//! Used in Mpx3GUI::save_data to send the save file path from the UI to
//! save_data. This is done after
...
void QCstmGLVisualization::StartDataTaking(QString mode) {
  ...
  if (!_dataTakingThread) {
    ...
    _dataTakingThread =
        new DataTakingThread(_mpx3gui, _dataConsumerThread, this);
...
void QCstmGLVisualization::dataTakingFinished() {
  ...
  _mpx3gui->GetSpidrController()->getSpidrReg(MPX3RX_UDP_PKTCNTR,
                                              &UDP_packetCounter);

  // qDebug().nospace() << "[DEBUG]\t0x" << hex << MPX3RX_UDP_PKTCNTR << " UDP
  // packet counter = " << dec << UDP_packetCounter;
  ...
  disconnect(this->getDataConsumerThread(), SIGNAL(doneWithOneFrame(int)),
             _mpx3gui->getCommandHandlerWrapper()->getCommandHandler(),
             SLOT(on_doneWithOneFrame(int)));
...
void QCstmGLVisualization::ConnectionStatusChanged(bool connecting) {
  ...
  if (connecting) {
    ...
    ui->startButton->setEnabled(true); // Enable or disable the button depending
    ...
    for (; i != iE; i++) {
      ...
      _statsString.devicesIdString.append(
          devID + _mpx3gui->getConfig()->getDeviceWaferId(indx));
      if ((i + 1) != iE)
        _statsString.devicesIdString.append(" | ");
    }
    ...
    _extraWidgets.devicesNamesLabel->setToolTip(
        tr("Device ID in [] followed by chip ID code"));
    int colCount = ui->dataTakingGridLayout->columnCount();
    ui->dataTakingGridLayout->addWidget(_extraWidgets.devicesNamesLabel, 1, 0,
                                        1, colCount);
...
void QCstmGLVisualization::SetMpx3GUI(Mpx3GUI *p) {
  ...
  connect(_mpx3gui, SIGNAL(sizeChanged(int, int)), ui->glPlot,
          SLOT(setSize(int, int)));
  connect(ui->startButton, SIGNAL(clicked(bool)), this,
          SLOT(StartDataTaking()));
  connect(this, SIGNAL(mode_changed(bool)), _mpx3gui, SLOT(set_summing(bool)));
  connect(_mpx3gui, SIGNAL(summing_set(bool)), ui->summingCheckbox,
          SLOT(setChecked(bool)));
  connect(ui->gradientSelector, SIGNAL(activated(int)), this,
          SLOT(setGradient(int)));
  connect(ui->generateDataButton, SIGNAL(clicked()), _mpx3gui,
          SLOT(generateFrame()));
  ...
  connect(_mpx3gui, SIGNAL(reload_all_layers()), this,
          SLOT(reload_all_layers()));
  connect(_mpx3gui, SIGNAL(availible_gradients_changed(QStringList)), this,
          SLOT(availible_gradients_changed(QStringList)));
  connect(ui->histPlot, SIGNAL(rangeChanged(QCPRange)), this,
          SLOT(range_changed(QCPRange)));

  connect(ui->binCountSpinner, SIGNAL(valueChanged(int)), this,
          SLOT(changeBinCount(int)));
  connect(ui->glPlot->getPlot(), SIGNAL(hovered_pixel_changed(QPoint)), this,
          SLOT(hover_changed(QPoint)));
  connect(ui->glPlot->getPlot(), SIGNAL(pixel_selected(QPoint, QPoint)), this,
          SLOT(pixel_selected(QPoint, QPoint)));
  connect(ui->glPlot->getPlot(),
          SIGNAL(region_selected(QPoint, QPoint, QPoint)), this,
          SLOT(region_selected(QPoint, QPoint, QPoint)));

  connect(this, SIGNAL(change_hover_text(QString)), ui->mouseOverLabel,
          SLOT(setText(QString)));
  connect(ui->histPlot, SIGNAL(new_range_dragged(QCPRange)), this,
          SLOT(new_range_dragged(QCPRange)));

  connect(this, &QCstmGLVisualization::sig_statusBarAppend, _mpx3gui,
          &Mpx3GUI::statusBarAppend);
  connect(this, &QCstmGLVisualization::sig_statusBarWrite, _mpx3gui,
          &Mpx3GUI::statusBarWrite);
  connect(this, &QCstmGLVisualization::sig_statusBarClean, _mpx3gui,
          &Mpx3GUI::statusBarClean);

  // Connection to configuration
  connect(ui->nTriggersSpinBox, SIGNAL(valueChanged(int)), this,
          SLOT(ntriggers_edit()));

  connect(ui->triggerLengthSpinBox, SIGNAL(valueChanged(double)), this,
          SLOT(triggerLength_edit()));

  // This one need both the connection to the mirror combo box and the signal to
  ...
  connect(ui->operationModeComboBox_Vis, SIGNAL(activated(int)),
          _mpx3gui->getConfig(), SLOT(setOperationMode(int)));

  // What to do over a switch of Operation Mode
  connect(ui->operationModeComboBox_Vis, SIGNAL(activated(int)), this,
          SLOT(OperationModeSwitched(int)));
  connect(ui->operationModeComboBox_Vis, SIGNAL(activated(int)),
          _mpx3gui->getConfigMonitoring(), SLOT(OperationModeSwitched(int)));

  // Double click to reset view
  // Linked to on_resetViewPushButton_clicked()
  connect(ui->glPlot->getPlot(), &QCstmGLPlot::double_click, this,
          &QCstmGLVisualization::reload_all_layers);
  connect(ui->resetViewPushButton, SIGNAL(pressed()), this,
          SLOT(on_resetViewPushButton_clicked()));

  // DataTaking / idling actions
  connect(this, SIGNAL(taking_data_gui()), _mpx3gui->getConfigMonitoring(),
          SLOT(when_taking_data_gui()));
  connect(this, SIGNAL(idling_gui()), _mpx3gui->getConfigMonitoring(),
          SLOT(when_idling_gui()));
  ...
  // TH Scan
  connect(this, SIGNAL(sig_resumeTHScan()), _mpx3gui->getTHScan(),
          SLOT(resumeTHScan()));

  // Getting equalization path
  connect(_mpx3gui->getEqualization(),
          SIGNAL(equalizationPathExported(QString)), this,
...
void QCstmGLVisualization::triggerLength_edit() {
  ...
  if (_mpx3gui->getConfig()->getOperationMode() ==
    ...
    // Send the new config
    _mpx3gui->getConfig()->setTriggerLength(ui->triggerLengthSpinBox->value());
...
void QCstmGLVisualization::fps_update(uint nframes_done) {
  ...
  const double fpsVal =
      (double(nframes_done)) /
      (double(_etatimer->elapsed()) / 1000.); // elapsed() comes in milliseconds
...
void QCstmGLVisualization::BuildStatsString() {
  ...
  //! Renable this if it turns out that the calculating the number of active
  //! pixels can be made faster See "BuildStatsStringCounts(
  //! _mpx3gui->getDataset()->getActivePixels(getActiveThreshold()) );"
...
void QCstmGLVisualization::predictAcquisitionTime() {
  ...
  if (config->getOperationMode() == Mpx3Config::__operationMode_SequentialRW) {
    ...
    singleFrameTime_us =
        double(1. / double(config->getContRWFreq()) * 1000000.);
  }

  time = time.addSecs(int(singleFrameTime_us / 1000000. *
                          config->getNTriggers() * (1 + __overhead)));
...
QString QCstmGLVisualization::getPath(QString msg) {
  ...
  if (autosaveFromServer) {
    ...
    path = QFileDialog::getExistingDirectory(this, msg, QDir::currentPath(),
...
void QCstmGLVisualization::user_accepted_profile() {
  ...
  if (_profiledialog) {
    ...
    // Re-initialize the Profilepoints list for new CNR calculation in new
    // dialog
...
void QCstmGLVisualization::OperationModeSwitched(int indx) {
  ...
  if (indx == Mpx3Config::__operationMode_SequentialRW) {
    ...
    ui->triggerLengthSpinBox->setValue(
        _mpx3gui->getConfig()->getTriggerLength_ms_64());
    ...
    ui->triggerLengthSpinBoxLabel->setToolTip(
        tr("ContinuousRW Mode. Enter frequency in Hz."));
    ui->triggerLengthSpinBox->setToolTip(
        tr("ContinuousRW Mode. Enter frequency in Hz."));
  }
}
...
void QCstmGLVisualization::hover_changed(QPoint pixel) {
  emit(change_hover_text(
      QString("%1 @ (%2, %3)")
          .arg(_mpx3gui->getPixelAt(pixel.x(), pixel.y(), getActiveThreshold()))
          .arg(pixel.x())
          .arg(pixel.y())));
}
...
void QCstmGLVisualization::reload_layer(int threshold) {
  ...
  ui->glPlot->getPlot()->readData(
      *_mpx3gui->getDataset()); // TODO: only read specific layer.

  ui->histPlot->setHistogram(threshold,
                             _mpx3gui->getDataset()->getLayer(threshold),
                             _mpx3gui->getDataset()->getPixelsPerLayer(),
                             _manualRange.lower, _manualRange.upper);
...
void QCstmGLVisualization::on_scoring(int nFramesReceived, int nFramesKept,
                                      int lostFrames, int lostPackets,
                                      int framesCount) {
...
void QCstmGLVisualization::setThreshold(int threshold) {
  ...
  // cout << "[INDEX] " << threshold << " --> " <<
  // _mpx3gui->getDataset()->thresholdToIndex(threshold) << endl;
  ui->layerSelector->setCurrentIndex(
      _mpx3gui->getDataset()->thresholdToIndex(threshold));
}
...
int QCstmGLVisualization::getActiveThreshold() {
  ...
  if (list.size() < 2)
    return -1;
  // The index will be found in the last position.  TODO ! Let's don't pick the
  // right threshold like this. Have a map !
...
void QCstmGLVisualization::shortcutStart() { ui->startButton->animateClick(); }
...
#endif
  //! Turn integration off, that is not valid when using
  //! takeAndSaveImageSequence
  _mpx3gui->set_summing(false);
...
void QCstmGLVisualization::setExposure(int microseconds) {
  ...
  //!    Always 2ms for down time, it's a 'safe' lower limit. 1 ms is likely
  //!    fine on good machines
  int index = -1;

  //! This method looks stupid and is slower but it is more robust to changes in
  //! the GUI.
  ...
  if (index >= 0) {
    ...
    emit someCommandHasFailed(
        QString("DEXTER --> ACQUILA ZMQ : Could not set exposure"));
  }
}
...
void QCstmGLVisualization::setThreshold(int threshold, int value) {
  ...
  //! TODO this operates on all chips simultaneously (check this), make is
  //! operate on a specified chip
...
void QCstmGLVisualization::setCSM(bool active) {
  ...
  if (active) {
    index =
        _mpx3gui->getConfigMonitoring()->getUI()->csmSpmCombo->findText("ON");
  } else {
    index =
        _mpx3gui->getConfigMonitoring()->getUI()->csmSpmCombo->findText("OFF");
  }
...
void QCstmGLVisualization::setReadoutMode(QString mode) {
  ...
  if ((mode.contains("srw")) || (mode.contains("seq"))) {
    index = _mpx3gui->getConfigMonitoring()
                ->getUI()
                ->operationModeComboBox->findText("Sequential R/W");
  } else if ((mode.contains("crw")) || (mode.contains("cont"))) {
    index = _mpx3gui->getConfigMonitoring()
                ->getUI()
                ->operationModeComboBox->findText("Continuous R/W");
  } else {
    qDebug() << "[ERROR]\tZMQ could not set readout mode : " << mode;
    emit someCommandHasFailed(
        QString("DEXTER --> ACQUILA ZMQ : Could not set readout mode"));
    return;
  }

  _mpx3gui->getConfigMonitoring()
      ->getUI()
      ->operationModeComboBox->setCurrentIndex(index);
#ifdef QT_DEBUG
...
void QCstmGLVisualization::loadConfiguration(QString filePath) {
  ...
  if (_mpx3gui->getConfig()->fromJsonFile(filePath, true)) {
    ...
    emit someCommandHasFailed(QString(
        "DEXTER --> ACQUILA ZMQ : Could not load configuration JSON file from" +
        filePath));
  }
}
...
void QCstmGLVisualization::setIntegration(bool integrate) {
  ...
  integrate ? onRequestForSettingFormatFromServer(0)
            : onRequestForSettingFormatFromServer(2);
...
void QCstmGLVisualization::onRequestForAutoSaveFromServer(bool val) {
  autosaveFromServer =
      true; //! This is so I don't have to modify on_saveCheckBox_clicked()
...
#ifdef QT_DEBUG
  qDebug() << "[INFO] QCstmGLVisualization::onRequestForAutoSaveFromServer ="
           << val;
#endif
}
...
void QCstmGLVisualization::region_selected(QPoint pixel_begin, QPoint pixel_end,
                                           QPoint position) {
  ...
  if (pixel_end.x() < 0 || pixel_end.y() < 0) {
    ...
    msgBox->setInformativeText(
        "You selected a pixel range with at least one negative end coordinate. "
        "\n\n Try again by ending your selection within the image.");
    msgBox->setDefaultButton(QMessageBox::Cancel);

    QSpacerItem *horizontalSpacer =
        new QSpacerItem(400, 0, QSizePolicy::Fixed, QSizePolicy::Fixed);
  ...
  int xImageLength =
      _mpx3gui->getDataset()->x() * _mpx3gui->getDataset()->getNChipsX();
  int yImageLength =
      _mpx3gui->getDataset()->y() * _mpx3gui->getDataset()->getNChipsY();
  ...
  QLabel *label =
      new QLabel(QString("\n    For region (%1, %2) --> (%3, %4) \n")
                     .arg(pixel_begin_checked.x())
                     .arg(pixel_begin_checked.y())
                     .arg(pixel_end_checked.x())
                     .arg(pixel_end_checked.y()),
                 this);
  ...
  // QAction calcStats(QString("Calculate statistics (%1, %2)-->(%3,
  // %4)").arg(pixel_begin.x()).arg(pixel_begin.y()).arg(pixel_end.x()).arg(pixel_end.y()),
  // &contextMenu);
  contextMenu.addAction(&calcStats);

  QAction calcProX(QString("ProfileX"), &contextMenu);
  // QAction calcProX(QString("ProfileX (%1, %2)-->(%3,
  // %4)").arg(pixel_begin.x()).arg(pixel_begin.y()).arg(pixel_end.x()).arg(pixel_end.y()),
  // &contextMenu);
  contextMenu.addAction(&calcProX);

  QAction calcProY(QString("ProfileY"), &contextMenu);
  // QAction calcProY(QString("ProfileY (%1, %2)-->(%3,
  // %4)").arg(pixel_begin.x()).arg(pixel_begin.y()).arg(pixel_end.x()).arg(pixel_end.y()),
  // &contextMenu);
  ...
  if (selectedItem == &calcStats) {
    ...
    // Calc basic stats
    _mpx3gui->getDataset()->calcBasicStats(pixel_begin_checked,
                                           pixel_end_checked);
    ...
    //_profiledialog->setAxisMap(_mpx3gui->getDataset()->calcProfile(axis,
    //threshold, pixel_begin_checked, pixel_end_checked));

    int layerIndex = getActiveThreshold();
    _profiledialog->setLayer(layerIndex);
    _profiledialog->setAxisMap(_mpx3gui->getDataset()->calcProfile(
        axis, layerIndex, pixel_begin_checked, pixel_end_checked));
...
//! TODO BUG The maths on this is incorrect, apparently opposing chips have
//! pixels masked when they should not be...
void QCstmGLVisualization::pixel_selected(QPoint pixel, QPoint position) {
  ...
  QPoint naturalCoords =
      _mpx3gui->getDataset()->getNaturalCoordinates(pixel, frameIndex);
  int naturalFlatCoord =
      naturalCoords.y() * _mpx3gui->getDataset()->x() + naturalCoords.x();
  if (_mpx3gui->getConfig()->getColourMode()) {
    naturalFlatCoord = 4 * naturalCoords.y() * _mpx3gui->getDataset()->x() +
                       2 * naturalCoords.x();
  }
  ...
  if (_maskingRequestFromServer) {
    ...
    QMenu contextMenu;
    QAction mask(QString("Mask pixel @ %1, %2").arg(pixel.x()).arg(pixel.y()),
                 &contextMenu),
        unmask(QString("Unmask pixel @ %1, %2").arg(pixel.x()).arg(pixel.y()),
               &contextMenu),
        maskAllOverflow(QString("Mask all pixels in overflow (per chip)"),
                        &contextMenu),
        maskAllActive(QString("Mask all active pixels (per chip)"),
                      &contextMenu);
    ...
    if (!_mpx3gui->equalizationLoaded()) {
      QMessageBox::information(nullptr, "Error",
                               "An equalisation must be loaded in order to "
                               "mask a pixel. Failed operation/");
      return;
    }
    ...
    if (selectedItem == &mask)
      ...
      _maskOperation = MASK_ALL_OVERFLOW;
    else if (selectedItem == &maskAllActive)
      _maskOperation = MASK_ALL_ACTIVE;
    else
      _maskOperation = NULL_MASK;
  }
  ...
  if (_maskOperation == MASK) {
  ...
  } else if ((_maskOperation == MASK_ALL_OVERFLOW) ||
             (_maskOperation == MASK_ALL_ACTIVE)) {
    ...
    for (; itr != itrE; ++itr) {
      ...
      if (colourMode) {
        naturalFlatCoords[chipId].push_back(4 * naturalCoords.y() * nx +
                                            2 * naturalCoords.x());
      } else {
        naturalFlatCoords[chipId].push_back(naturalCoords.y() * nx +
                                            naturalCoords.x());
      }
    }
    ...
    for (; itrM != itrME; ++itrM) {
      ...
      auto eqResChip =
          _mpx3gui->getEqualization()->GetEqualizationResults(chip1);
  ...
  _mpx3gui->getEqualization()->SetAllAdjustmentBits(
      _mpx3gui->getConfig()->getController(), chip, true);
}
...
void QCstmGLVisualization::on_manualRangeRadio_toggled(bool checked) {
  ...
  if (checked) {
    ...
    if (activeTHL >= 0) {
      // Throw a recomendation here to the user.  If manual has never been set,
      // then use
...
void QCstmGLVisualization::on_percentileRangeRadio_toggled(bool checked) {
  ...
  if (checked) {
    ...
    _percentileRangeNatural = ui->histPlot->set_scale_percentile(
        getActiveThreshold(), ui->lowerSpin->value(), ui->upperSpin->value());
    ui->histPlot->scaleToInterest();
  }
}
...
void QCstmGLVisualization::on_layerSelector_activated(const QString &arg1) {
  ...
  int layer = _mpx3gui->getDataset()->thresholdToIndex(threshold);
#ifdef QT_DEBUG
  qDebug() << "[DEBUG] INDEX" << threshold << "-->" << layer << "from list"
           << _mpx3gui->getDataset()->getThresholds() << "\n";
#endif
...
void QCstmGLVisualization::on_infDataTakingCheckBox_toggled(bool checked) {
  ...
  if (checked) {
    ...
    // In this case configure the system for 0 triggers.  It will do for both
    // operation modes
...
void QCstmGLVisualization::consumerFinishedOneFrame(int frameId) {
  ...
  if (_saveCheckBox_isChecked && _saveLineEdit_isNotEmpty &&
      _saveAllCheckBox_isChecked) {
...
void QCstmGLVisualization::consumerBufferFull(int) {
  QMessageBox::critical(
      this, tr("System buffer"),
      tr("The system can't keep up. Please review your settings. Increasing "
         "UDP kernel buffer size or getting a faster CPU may help."));
}
...
void QCstmGLVisualization::on_saveLineEdit_textEdited() {
  ...
  if (dir.exists()) {
    ...
    if (dir_info.isWritable()) {
      ui->saveLineEdit->setStyleSheet(
          "QLineEdit { background: rgb(255, 255, 255); "
          "selection-background-color: rgb(0, 80, 80); }");
    ...
    if (!dir_info.isWritable()) {
      ...
      ui->saveLineEdit->setStyleSheet(
          "QLineEdit { background: rgb(150, 240, 150); "
          "selection-background-color: rgb(50, 150, 50); }");
      _saveLineEdit_isNotEmpty = !ui->saveLineEdit->text().isEmpty();
      // Does not exist and is not writable --> red
    } else {
      const QString msg =
          "Path does not exist and is not writable (yet): " + path;
      emit sig_statusBarAppend(msg, "red");
      qDebug().noquote() << "[INFO]\t" << msg;
...
void QCstmGLVisualization::onPixelsMasked(int devID, QSet<int> pixelSet) {
  /* So that the equalisation won't save the mask file to the wrong folder
   * during equalisation */
  if (_equalisationPath != "") {

    qDebug() << "[INFO]\tPixels loaded/masked, making a file in "
             << _equalisationPath;
...
bool QCstmGLVisualization::requestToSetSavePath(QString path) {
  /* Check the path exists, try to create it, otherwise set it to the home
   * directory. */
  ...
  if (!dir_info.isWritable()) {
    ...
    const QString msg =
        "Path was not writable, setting autosave path to home directory";
    emit sig_statusBarAppend(msg, "black");
    qDebug().noquote() << "[INFO]\t" << msg;
  }
  _saveLineEdit_isNotEmpty = !ui->saveLineEdit->text().isEmpty();
...
void QCstmGLVisualization::_loadFromThresholdsVector() {
  for (int chip = 0; chip < __max_number_of_chips; ++chip) {
    for (int DAC_index = 0; DAC_index < __max_colours; ++DAC_index) {
      _mpx3gui->getDACs()->setDAC_index(nullptr, chip, DAC_index,
                                        _thresholdsVector[chip][DAC_index]);
    }
  }
}
...
void QCstmGLVisualization::initialiseThresholdsVector() {
  for (int chip = 0; chip < __max_number_of_chips; ++chip) {
    for (int DAC_index = 0; DAC_index < __max_number_of_thresholds;
         ++DAC_index) {
      _thresholdsVector[chip][DAC_index] =
          _mpx3gui->getConfig()->getDACValue(uint(chip), DAC_index);
    }
  }
}
...
void QCstmGLVisualization::on_saveFileComboBox_currentIndexChanged(
