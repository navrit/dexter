...
void byeHandler(CommandHandler *ch, Command *cmd) {
  ...
  QString tstData =
      ...
      "scrambled it to make a type ki";
...
void setReadoutModeHandler(CommandHandler *ch, Command *cmd) {
  Q_UNUSED(ch);
  if (!cmd->enoughArguments(
          1, "SetReadoutMode")) // this command comes with one argument
  {
...
void setCounterDepthHandler(CommandHandler *ch, Command *cmd) {
  Q_UNUSED(ch);
  if (!cmd->enoughArguments(
          1, "SetCounterDepth")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setOperationalModeHandler(CommandHandler *ch, Command *cmd) {
  Q_UNUSED(ch);
  if (!cmd->enoughArguments(
          1, "SetOperationalMode")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setTriggerModeHandler(CommandHandler *ch, Command *cmd) {
   ...
   */
  if (!cmd->enoughArguments(
          1, "SetTriggerMode")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setGainModeHandler(CommandHandler *ch, Command *cmd) {
  Q_UNUSED(ch);
  if (!cmd->enoughArguments(
          1, "SetGainMode")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setPolarityHandler(CommandHandler *ch, Command *cmd) {
  Q_UNUSED(ch);
  if (!cmd->enoughArguments(
          1, "SetPolarity")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setCounterSelectFrequencyHandler(CommandHandler *ch, Command *cmd) {
  ...
  if (!cmd->enoughArguments(
          1,
          "SetCounterSelectFrequency")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setShutterLengthHandler(CommandHandler *ch, Command *cmd) {
  Q_UNUSED(ch);
  if (!cmd->enoughArguments(
          2, "SetShutterLength")) // this command comes with two argument
                                  // {SetShutterLength;{open,down};value
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
  ...
  auto config = Mpx3GUI::getInstance()->getConfig();
  bool cont =
      (config->getOperationMode() == Mpx3Config::__operationMode_ContinuousRW);
  if (openOrClose == "open") {
    // here code to set operational mode
    if (cont) {
...
void setShutterPeriodHandler(CommandHandler *ch, Command *cmd) {
  Q_UNUSED(ch);
  if (!cmd->enoughArguments(
          1, "SetShutterPeriod")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void getShutterLengthHandler(CommandHandler *ch, Command *cmd) {
  Q_UNUSED(ch);
  if (!cmd->enoughArguments(
          1, "GetShutterLength")) // this command comes with two argument
                                  // {GetShutterLength;{open,down}
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
  ...
  if (openOrClose == "open") {
    ...
    double trig = cont ? 0.0 : config->getTriggerDowntime_ms_64(); // ms
    cmd->setData(QString::number(trig));
    cmd->setError(NO_ERROR);
  } else {
    cmd->setError(UNKNOWN_COMMAND);
...
void setBothCountersHandler(CommandHandler *ch, Command *cmd) {
  ...
  if (!cmd->enoughArguments(
          1, "SetBothCounters")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
  ...
  if (cmd->arguments.at(0) == "2") { // we send the data of both counter
    ...
    cmd->setError(NO_ERROR);
  } else if (cmd->arguments.at(0) == "0" ||
             cmd->arguments.at(0) == "1") { // we send the data of low counter
...
void setColourModeHandler(CommandHandler *ch, Command *cmd) {
  ...
  if (!cmd->enoughArguments(
          1, "SetColourMode")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setChargeSummingHandler(CommandHandler *ch, Command *cmd) {
  ...
  if (!cmd->enoughArguments(
          1, "SetChargeSumming")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setAutoSaveHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(
          1, "SetAutoSave")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setRecordPathHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(
          1, "SetRecordPath")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setRecordFormatHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(
          1, "SetRecordFormat")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
//    Q_UNUSED(ch);
//    // TODO: this function is broken: it will not produce an image in an
//    understandable format

////    QVector<int> frame0 =
///Mpx3GUI::getInstance()->getDataset()->makeFrameForSaving(0,false); /   //
...
//    for(int i = 0; i<20; i++){
//        int value = 0;
//        value |= im.at(i*4) | (im.at((i*4)+1)<<8) | (im.at((i*4)+2)<<16) |
//        (im.at((i*4)+3)<<24); strData += QString::number(value) + ";";
//    }
//  //  cmd->setImage();

//    QByteArray pixels = im.mid(20*4); //17*4 = 68 bytes header later this must
//    become dynamic // 18*4 = 72 bytes for double counter qDebug()<<"pixel
...
void setNumberOfFrameHandler(CommandHandler *ch, Command *cmd) {
  Q_UNUSED(ch);
  if (!cmd->enoughArguments(
          1, "SetFrameNumber")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void getNumberOfFrameHandler(CommandHandler *ch, Command *cmd) {
  Q_UNUSED(ch);
  cmd->setData(
      QString::number(Mpx3GUI::getInstance()->getConfig()->getNTriggers()));
  cmd->setError(NO_ERROR);
}
...
void setThresholdHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(2, "SetThreshold")) // this command comes with two
                                                // argument{SetThreshold;0;100}
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
  int threshold = cmd->arguments.at(0).toInt();
  double energy = cmd->arguments.at(1).toDouble(); // energy kev

  int dac = Mpx3GUI::getInstance()->getEnergyCalibrator()->calcDac(0, threshold, energy);
  saturateThresholdDacValue(&dac);
  ch->setThreshold(threshold, dac, 0);

  qDebug() << "[INFO]\tThreshold " << threshold
           << " of chip 0 converted to : " << dac;

  dac = Mpx3GUI::getInstance()->getEnergyCalibrator()->calcDac(1,  threshold, energy);
  saturateThresholdDacValue(&dac);
  ch->setThreshold(threshold, dac, 1);

  qDebug() << "[INFO]\tThreshold " << threshold
           << " of chip 1 converted to : " << dac;

  dac = Mpx3GUI::getInstance()->getEnergyCalibrator()->calcDac(2,  threshold, energy);
  saturateThresholdDacValue(&dac);
  ch->setThreshold(threshold, dac, 2);

  qDebug() << "[INFO]\tThreshold " << threshold
           << " of chip 2 converted to : " << dac;
  dac = Mpx3GUI::getInstance()->getEnergyCalibrator()->calcDac(3,  threshold, energy);
  saturateThresholdDacValue(&dac);
  ch->setThreshold(threshold, dac, 3);

  qDebug() << "[INFO]\tThreshold " << threshold
           << " of chip 3 converted to : " << dac;
...
void getThresholdHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(
          1, "GetThreshold")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setThresholdPerChipHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(
          3, "SetThresholdPerChip")) // this command comes with three
                                     // argument{SetThresholdPerChip;th;chip;val}
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
  ...
  // No need for the conversion here. Per chip we address the setting in DAC
  // units.
  // int dac =
  // Mpx3GUI::getInstance()->getEnergyCalibrator()->calDac(chipId,val);
...
void getThresholdPerChipHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(
          2, "GetThresholdPerChip")) // this command comes with two
                                     // argument{SetThresholdPerChip;th;chip}
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
  ...
  // double energy =
  // Mpx3GUI::getInstance()->getEnergyCalibrator()->calcEnergy(chipId,val);
...
void setStartScanHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(
          1, "SetStartScan")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setStopScanHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(
          1, "SetStopScan")) // this command comes with one argument
    return;
  int val = int(cmd->arguments.at(0).toDouble());
...
void setStepScanHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(
          1, "SetStepScan")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setThresholdScanHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(
          1, "SetThresholdScan")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setFramesPerScanHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(
          1, "SetFramesPerScan")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setScanPathHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(
          1, "SetScanPath")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setOperatingEnergyHandler(CommandHandler *ch, Command *cmd) {
  Q_UNUSED(ch);
  if (!cmd->enoughArguments(
          1, "SetOperatingEnergy")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setProfileHandler(CommandHandler *ch, Command *cmd) {
  Q_UNUSED(ch);
  if (!cmd->enoughArguments(
          1, "SetProfile")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setMaskPixelHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(
          2, "SetMaskPixel")) // this command comes with two argument x,y
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setUnmaskPixelHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(
          2, "SetUnmaskPixel")) // this command comes with two argument x,y
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setEqualizationHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(
          1, "SetEqualization")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setDoEqualizationHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(
          1, "SetDoEqualization")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setConfigHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(
          1, "SetConfig")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void saveConfigHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(
          1, "SaveConfig")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setInhibitShutterHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(
          1, "SetInhibitShutter")) // this command comes with one argument
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setSlopeHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(
          2, "SetSlope")) // this command comes with two arguments (chipNum,val)
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
  int error = ch->setSlope(cmd->arguments.at(0).toInt(),
                           cmd->arguments.at(1).toDouble());
  cmd->setData("1");
  cmd->setError(ERROR_TYPE(error));
}
...
void getSlopeHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(
          1, "GetSlope")) // this command comes with one argument (chipNum)
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
...
void setOffsetHandler(CommandHandler *ch, Command *cmd) {
  if (!cmd->enoughArguments(
          2,
          "SetOffset")) // this command comes with two arguments (chipNum,val)
  {
    cmd->setError(ARG_NUM_OUT_RANGE);
    return;
  }
  int error = ch->setOffset(cmd->arguments.at(0).toInt(),
                            cmd->arguments.at(1).toDouble());
  cmd->setData("1");
  cmd->setError(ERROR_TYPE(error));
}
