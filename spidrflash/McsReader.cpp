...
McsReader::McsReader()
    : _valid(false), _minAddr(-1), _maxAddr(-1), _lineNo(0), _mem(nullptr) {
...
unsigned char *McsReader::page(int page_no) {
...
void McsReader::setFile(std::string filename) {
  ...
  while (fgets(inp, sizeof(inp), fp) != nullptr) {
    ++line_no;
    ...
    if (*pch == ':') {
      ...
      try {
        this->interpretHexline(pch, &no_of_bytes, &addr, &hextype, databytes,
                               &checksum);
      ...
      case 0x01:
        ...
        if (no_of_bytes != 0 || addr != 0) {
          fclose(fp);
          McsException exc("error in End-of-Data (RecType 1) ", line_no);
          throw exc;
        }
      ...
      case 0x02:
        ...
        if (no_of_bytes != 2 || addr != 0) {
          fclose(fp);
          McsException exc("error in Ext Segm Addr (RecType 2) ", line_no);
          throw exc;
        } else {
          segoffset = ((databytes[0] << 8) | databytes[1]) * 16;
          if (segoffset > 0x10000) {
            fclose(fp);
            McsException exc("illegal Extended Segm Addr (RecType", line_no,
                             segoffset);
            throw exc;
          }
      ...
      case 0x04:
        ...
        if (no_of_bytes != 2 || addr != 0) {
          fclose(fp);
          McsException exc("error in Ext Linear Addr (RecType 4) ", line_no);
          throw exc;
        } else {
          segoffset = ((databytes[0] << 8) | databytes[1]) << 16;
          if (segoffset > MEM_SZ - 0x10000) {
            fclose(fp);
            McsException exc("illegal Ext Linear Addr (RecType 4)", line_no,
                             segoffset);
            throw exc;
          }
...
void McsReader::interpretHexline(char *hexline, byte *pno_of_bytes, int *paddr,
                                 byte *phextype, byte *databytes,
                                 byte *pchecksum) {
...
McsException::McsException(std::string str) : _str(str), _lineNo(0), _data(0) {}
...
void McsException::setLineNo(int line_no) { _lineNo = line_no; }
